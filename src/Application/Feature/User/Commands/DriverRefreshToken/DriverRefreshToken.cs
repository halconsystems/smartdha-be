...
namespace DHAFacilitationAPIs.Application.Feature.User.Commands.DriverRefreshToken;
public record DriverRefreshTokenCommand(string AccessToken)
    : IRequest<SuccessResponse<MobileAuthenticationDto>>;
public class RefreshTokenHandler
    : IRequestHandler<DriverRefreshTokenCommand, SuccessResponse<MobileAuthenticationDto>>
{
    ...
    public async Task<SuccessResponse<MobileAuthenticationDto>> Handle(
        DriverRefreshTokenCommand request, CancellationToken cancellationToken)
    {
        ...
        string newAccessToken = await _authService.GenerateDriverToken(user);
