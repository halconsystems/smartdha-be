...
public record GetAllGroundBookingQuery : IRequest<List<BookingDTO>>;
public class GetAllGroundBookingQueryHandler : IRequestHandler<GetAllGroundBookingQuery, List<BookingDTO>>
{
    private readonly IOLMRSApplicationDbContext _context;
    private readonly ICurrentUserService _currentUser;

    public GetAllGroundBookingQueryHandler(IOLMRSApplicationDbContext context, ICurrentUserService currentUser)
    {
        _context = context;
        _currentUser = currentUser;
    }

    public async Task<List<BookingDTO>> Handle(GetAllGroundBookingQuery request, CancellationToken ct)
    {
        ...
        var booking = await _context.GroundBookings.Where(x => x.UserId == UserID)
            .Include(x => x.Grounds)
            .AsNoTracking()
            .ToListAsync(ct);
