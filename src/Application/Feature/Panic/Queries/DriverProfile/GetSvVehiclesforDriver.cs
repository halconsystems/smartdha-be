...
public record GetSvVehiclesforDriverQuery() : IRequest<List<SvVehicleListDto>>;

public class GetSvVehiclesforDriverQueryHandler
    : IRequestHandler<GetSvVehiclesforDriverQuery, List<SvVehicleListDto>>
{
    private readonly IApplicationDbContext _context;
    private readonly UserManager<ApplicationUser> _userManager;

    public GetSvVehiclesforDriverQueryHandler(
    ...
    public async Task<List<SvVehicleListDto>> Handle(GetSvVehiclesforDriverQuery request, CancellationToken ct)
    {
        var vehicles = await _context.SvVehicles
            .Include(v => v.SvPoint)
            .Where(v => v.IsActive == true && v.IsDeleted==false && v.DriverUserId == null && v.Status == SvVehicleStatus.Available)
            .ToListAsync(ct);
