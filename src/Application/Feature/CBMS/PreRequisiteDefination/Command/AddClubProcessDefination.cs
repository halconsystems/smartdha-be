...
public record AddClubProcessDefinationCommand(
    string Name,
    string Code,
...
public class AddClubProcessDefinationCommandHandler
    : IRequestHandler<AddClubProcessDefinationCommand, ApiResult<Guid>>
{
    private readonly IOLMRSApplicationDbContext _db;

    public AddClubProcessDefinationCommandHandler(IOLMRSApplicationDbContext db)
    {
        _db = db;
    }

    public async Task<ApiResult<Guid>> Handle(
        AddClubProcessDefinationCommand r,
