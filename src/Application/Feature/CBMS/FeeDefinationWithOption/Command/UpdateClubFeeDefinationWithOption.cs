...
using DHAFacilitationAPIs.Application.Feature.CBMS.ClubFeeOption.Command;
...
public record UpdateClubFeeDefinationWithOptionsCommand(
    ...
    List<ClubFeeOptionInput>? Options
) : IRequest<ApiResult<bool>>;

public class UpdateClubFeeDefinationWithOptionsCommandHandler
    : IRequestHandler<UpdateClubFeeDefinationWithOptionsCommand, ApiResult<bool>>
{
    private readonly IOLMRSApplicationDbContext _db;

    public UpdateClubFeeDefinationWithOptionsCommandHandler(IOLMRSApplicationDbContext db)
    {
        _db = db;
    }

    public async Task<ApiResult<bool>> Handle(
        UpdateClubFeeDefinationWithOptionsCommand r,
        CancellationToken ct)
    {
        // 1️⃣ Load FeeDefinition (tracked)
        var feeDef = await _db.Set<ClubFeeDefinition>()
        ...
        var existingOptions = await _db.Set<Domain.Entities.CBMS.ClubFeeOption>()
            .Where(x =>
        ...
        if (isOptionBased)
        {
            ...
            _db.Set<Domain.Entities.CBMS.ClubFeeOption>().AddRange(newOptions);
        }
