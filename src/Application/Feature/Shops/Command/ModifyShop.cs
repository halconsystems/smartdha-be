...
public record UpdateShopCommand(Guid ShopId, string name, string DisplayName, string Address, float Latitude, float Longitude, string OwnerName, string OwnerEmail, string OwnerPhone, string ShopPhoneNumber)
    : IRequest<SuccessResponse<string>>;

public class UpdateShopCommandHandler
    : IRequestHandler<UpdateShopCommand, SuccessResponse<string>>
{
    private readonly ILaundrySystemDbContext _context;
    private readonly ICurrentUserService _currentUserService;

    public UpdateShopCommandHandler(ILaundrySystemDbContext context, ICurrentUserService currentUserService)
    {
        _context = context;
        _currentUserService = currentUserService;
    }

    public async Task<SuccessResponse<string>> Handle(UpdateShopCommand request, CancellationToken cancellationToken)
    {
