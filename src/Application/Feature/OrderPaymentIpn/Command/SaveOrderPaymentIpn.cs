...
namespace DHAFacilitationAPIs.Application.Feature.OrderPaymentIpn.Command;

public record SaveOrderPaymentIpnCommand(
    PaymentIpnRequestDto Payload,
    string RawPayload
) : IRequest<Guid>;
public class SaveOrderPaymentIpnCommandHandler
    : IRequestHandler<SaveOrderPaymentIpnCommand, Guid>
{
    private readonly ILaundrySystemDbContext _ctx;

    public SaveOrderPaymentIpnCommandHandler(ILaundrySystemDbContext ctx)
    {
        _ctx = ctx;
    }

    public async Task<Guid> Handle(SaveOrderPaymentIpnCommand request, CancellationToken ct)
    {
        ...
        var entity = new OrderPaymentIpnLogs
        {
            Id = Guid.NewGuid(),
        ...
        _ctx.OrderPaymentIpnLogs.Add(entity);
