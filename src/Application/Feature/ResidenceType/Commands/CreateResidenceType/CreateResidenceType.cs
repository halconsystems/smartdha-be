...
public record CreateResidenceTypeCommand(string Name, string? Description, ClubType ClubType, Guid ClubId)
...
public class CreateResidenceTypeCommandHandler : IRequestHandler<CreateResidenceTypeCommand, SuccessResponse<string>>
{
    ...
    public CreateResidenceTypeCommandHandler(IOLMRSApplicationDbContext ctx, IApplicationDbContext appCtx, ICurrentUserService currentUser)
    {
        _ctx = ctx;
        _appCtx = appCtx;
