...
public class SvVehicle : BaseAuditableEntity
{
    ...
    public Guid SvPointId { get; set; }
    public SvPoint SvPoint { get; set; } = default!;

    // OPTIONAL: which user (driver) owns this vehicle in the mobile app
    public string? DriverUserId { get; set; }  // FK to AspNetUsers.Id (string)
    // public ApplicationUser? Driver { get; set; }

    // Live tracking (optional, from driver app)
    public decimal? LastLatitude { get; set; }   // decimal(9,6)
    public decimal? LastLongitude { get; set; }  // decimal(9,6)
